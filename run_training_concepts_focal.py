...
class FocalLoss(nn.Module):
    ...
    -----
    alpha : float
        Balancing factor (weight for positive class). 0 < alpha ≤ 1.
    gamma : float
        Focusing parameter ≥ 0. When 0 this reduces to BCE.
    reduction : str
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:  # type: ignore[override]
        ...
        p_t = p * targets + (1 - p) * (1 - targets)  # probability for the ground‑truth class
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--loss", choices=["bce", "focal"], default="bce", help="Loss function to use (default=bce)")
    parser.add_argument("--focal_alpha", type=float, default=0.25, help="Alpha parameter for Focal Loss (ignored for BCE)")
    parser.add_argument("--focal_gamma", type=float, default=2.0, help="Gamma parameter for Focal Loss (ignored for BCE)")
